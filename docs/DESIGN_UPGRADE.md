...
# 藝術方向定案：「燈箱圖譜」（2026-08-25 三方合議）

> 過程：主編提案（雙材質）→ codex／grok 各自獨立評審（全文在 `course/research/design-critique-{codex,grok}.md`）→ 本節為整合裁決。

## 合議結論

**品牌識別 = 標註文法＋聲窗線稿；設計系統全部退居家具。**
截一張帶標註的影像、關掉網址列，仍認得出是這個課——這是唯一目標。

## 定案 token

| 角色 | Hex | 規則 |
|---|---|---|
| 紙面（閱讀層） | `#F2EFE6` | 乾銅版紙，不是奶油白；薄線分隔，不堆卡片 |
| 墨色 | `#1A2330` | 冷墨；**廢除品牌 navy** |
| 燈箱（影像層） | `#0C0E12` | 永遠中性黑、不隨主題反相；非純黑（保 8-bit 暗部） |
| 教學標記黃 | `#E0B000` | **只**給目標結構：實線箭頭、卡尺、ROI 外框 |
| 陷阱洋紅 | `#C43B6E` | 虛線／斜線填充：假象、正常變異、勿過度診斷 |
| 互動 teal | `#1B6B73` | 去飽和、讓路給標註；紙面/燈箱同色相兩套亮度 |

禁忌紅 `#B42318` 只給安全事項。標註三色必須通過灰階／Doppler／X 光三關對比測試。

## 字體定案

- **不導入 Noto Serif TC**（兩位評審一致：台灣學術站預設套餐，且與雙材質的權威訊號互踩）
- 正文／UI：Noto Sans TC 400/500/700（**須實際載入 webfont**——現站只宣告未載入，跨平台不一致）
- 標題權威來自層級與留白：單元名 Noto Sans TC 500、32–40px
- 資料（時間碼/測量/序列參數/證據等級）：IBM Plex Mono＋tabular-nums

## 標註文法（品牌核心，三條不可破）

1. 目標結構＝鎘黃**實線**（箭頭/卡尺/ROI）
2. 邊界＝同色**虛線**
3. 陷阱＝洋紅**斜線填充**；一律附文字標籤，不只靠顏色（色盲安全）

## 聲窗線稿（8–12 枚，不取代 Lucide）

膝輪廓＋探頭/束向/切面的 3 筆線稿，鎖在每張影像同一角落與單元目錄；左右側、探頭 notch、長短軸、矢冠軸切面**需醫師審核**（畫錯是教學錯誤）。UI chrome 繼續用 Lucide。

## 動效白名單（只有三個）

cine 實際播放、標註在「先看圖」後淡入、quiz 對錯回饋。**掃描線 loading 廢除**（HUD 迷因，非診斷語意）。

## 實施順序

1. **S**：token 換裝（紙/墨/燈箱三色）、廢 navy 殘留、webfont 實際載入、mono 化資料、動效白名單
2. **M**：聲窗線稿 8–12 枚（插畫＋醫師審核）、hero 重排（真實影像跨紙面與燈箱＋角落線稿＋單一黃標註）
3. **L**：overlay 標註系統上真實影像（依賴 Phase 3 素材裁決）——**hero 用真實影像是前提，CSS 假超音波是全站最削弱專業感的元素**（codex）
